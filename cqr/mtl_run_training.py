...
def train(args, train_dataset, model, tokenizer, logger, cross_validate_id=-1):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            mc_token_ids = torch.tensor([0 for _ in inputs])
            model.train()
            # print(f"inputs: {inputs.shape}, lm_labels: {labels.shape}, mc_labels: {mc_labels.shape}")
            # exit(0)
            ...
            # print(outputs[0],outputs[1],outputs[2].shape, outputs[3].shape)
